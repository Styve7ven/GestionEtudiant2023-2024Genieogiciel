...
namespace GestionPersonneLib
{
    public class Telephone : ITelephone
    {
        ...
        public List<ITelephone> TelephonesPersonnes(string id_personne)
        {
            List<ITelephone> lst = new List<ITelephone>();
            ...
            using (IDbCommand cmd = ImplementeConnexion.Instance.Conn.CreateCommand())
            {
                cmd.CommandText = "sp_select_telephones_etudiant";
                cmd.CommandType = CommandType.StoredProcedure;

                cmd.Parameters.Add(Parametres.Instance.AjouterParametre(cmd, "@id_etudiant", 4, DbType.String, id_personne));
